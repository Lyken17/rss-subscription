...
if __name__ == "__main__":
    ...
    out = xmltodict.parse(open("rss/mikan-today.xml", mode="r", encoding="utf-8").read())
    xmltodict.unparse(out, output=open("rss/mikan-today.xml", mode="w", encoding="utf-8"), pretty=True)
    
    # add all items
    with open("rss/all_items.json", "r", encoding="utf-8") as file:
    ...
    template_rss = xmltodict.parse(open("rss/mikan-today.xml", mode="r", encoding="utf-8").read())
    for k, items in new_items.items():
        ...
        xmltodict.unparse(out, output=open(f"rss/mikan-recent-{k}.xml", mode="w", encoding="utf-8"), pretty=True)
        ...
        xmltodict.unparse(out, output=open(f"rss/lolihouse-recent-{k}.xml", mode="w", encoding="utf-8"), pretty=True)
